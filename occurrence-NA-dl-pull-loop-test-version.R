...
test_list <- name_list[sapply(name_list, function(x) any(grepl("Osmia ashmeadii", x)))]

accepted_name_v <- accepted_name_v[str_detect(accepted_name_v, "Osmia ashmeadii")] # For now check with a known output species 
...
for(i in 1:length(accepted_name_v)){
 ...
 # FLAG records that are problematic for dupes 
  # locality_clean_holder$dup_flag <- ifelse(all(is.na(locality_clean_holder$year)) && 
  #                                            all(is.na(locality_clean_holder$month)) && 
  #                                            all(is.na(locality_clean_holder$day)), 
  #                                          TRUE, FALSE)
  if (
    !all(is.na(locality_clean_holder$year)) && 
    !all(is.na(locality_clean_holder$month)) && 
    !all(is.na(locality_clean_holder$day))) {
  duplicate_clean_holder <- remove_duplicates(df = locality_clean_holder,
    ...
    duplicate_clean_holder <- data.frame(accepted_name = accepted_name_v[[i]], failure = "Not Enough Data to resolve duplicates")
  }
  
  if(!"failure" %in% colnames(duplicate_clean_holder)){
  specimen_clean_holder <- basis_clean(df = duplicate_clean_holder,
  ...
  specimen_analysis_dataset <- specimen_clean_holder # Reassign Name 
...
# 4. Create an output table noting some diagnostics: Here we're adding how many records are removed per step by AcceptedName species pull. 
  ...
  gators_cleaning[i, 2] <- nrow(taxa_clean_holder) # Store the number of occurrences after taxa cleaning
  gators_cleaning[i, 3] <- nrow(locality_clean_holder) # Store the number of occurrences after locality cleaning 
  gators_cleaning[i, 4] <- nrow(duplicate_clean_holder) # Store the number of occurrences after duplicate cleaning 
  gators_cleaning[i, 5] <- nrow(specimen_clean_holder) # Store the number of occurrences after clarifying basisOfRecord
  gators_cleaning[i, 6] <- nrow(subset(gators.output, aggregator == "iDigBio")) # How many records for aggregator iDigBio
  gators_cleaning[i, 7] <- nrow(subset(gators.output, aggregator == "GBIF")) # How many records for aggregator GBIF
  ...
  colnames(gators_cleaning)[5] = "basisOfRecordClean"
  colnames(gators_cleaning)[6] = "iDigBioTotalRecordsPull"
  colnames(gators_cleaning)[7] = "GBIFTotalRecordsPull"
  
cleaning_path <- "/home/jt-miller/Pollen-Project/pollen-project-mapping/test-data-outputs-gators/cleaning-tables/"
write.table(gators_cleaning, file=paste0(cleaning_path, "cleaning-steps-", accepted_name_v_filestyle[[i]],".txt"), row.names = FALSE, sep="\t", quote = FALSE)
...
write.table(bees_regioned_df, file=paste0(path, accepted_name_v_filestyle[[i]],".txt"), row.names = FALSE, sep="\t", quote = FALSE)
    ...
    # Note the gators cleaning where things go wrong
    gators_cleaning[i, 1] <- accepted_name_v[[i]] # Store the AcceptedName as the one we identify the species by
    gators_cleaning[i, 2] <- nrow(taxa_clean_holder) # Store the number of occurrences after taxa cleaning
    gators_cleaning[i, 3] <- nrow(locality_clean_holder) # Store the number of occurrences after locality cleaning
    gators_cleaning[i, 4] <- "FAILED" # Store the number of occurrences after duplicate cleaning 
    gators_cleaning[i, 5] <- NA # Store the number of occurrences after clarifying basisOfRecord
    gators_cleaning[i, 6] <- nrow(subset(gators.output, aggregator == "iDigBio")) # How many records for aggregator iDigBio
    gators_cleaning[i, 7] <- nrow(subset(gators.output, aggregator == "GBIF")) # How many records for aggregator GBIF
    colnames(gators_cleaning)[1] = "acceptedName" # Rename these cols for clarity
    colnames(gators_cleaning)[2] = "pulledRecords"
    ...
    path <- "/home/jt-miller/Pollen-Project/pollen-project-mapping/test-data-outputs-gators/specimen-info-datasets/"
    write.table(duplicate_clean_holder, file=paste0(path, accepted_name_v_filestyle[[i]],".txt"), row.names = FALSE, sep="\t", quote = FALSE)
    cleaning_path <- "/home/jt-miller/Pollen-Project/pollen-project-mapping/test-data-outputs-gators/cleaning-tables/"
    write.table(gators_cleaning, file=paste0(cleaning_path, "cleaning-steps-", accepted_name_v_filestyle[[i]],".txt"), row.names = FALSE, sep="\t", quote = FALSE)
...
# Check output 
check <- fread("/home/jt-miller/Pollen-Project/pollen-project-mapping/test-data-outputs-gators/specimen-info-datasets/Anthophora-hololeuca.txt")
